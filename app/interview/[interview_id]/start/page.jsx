...
function StartInterview() {
  ...
  // Initialized as an array to correctly store conversation history
  ...
  useEffect(() => {
    ...
    const handleMessage = (message) => {
      ...
      if (message?.conversation) {
        // Store the array of message objects directly
  ...
  const startCall = async () => {
    if (interviewInfo?.participants) {
      interviewInfo.participants.forEach((participant) => {
        ...
        // Assuming sendNotification is defined elsewhere
...
`.trim(),
  ...
  const GenerateFeedback = async () => {
    setLoading(true);
    try {
      // The conversation is already a proper array, no need to stringify.
      ...
      try {
        // The AI response is a stringified JSON, so we parse it
        ...
        .insert([
          {
            ...
            feedback: JSON.stringify(feedbackData), // Supabase expects a string for a JSON column
            recommended: false,
          },
        ])
        .select();
